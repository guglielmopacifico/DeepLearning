...
class FBPINN_Cos_nD(nn.Module):
    """
    This class implements the Finite Basis PINN (FBPINN_Cos_nD), to solve the problem that the vanilla PINN is not able to solve.
    ...
    def loss_function(self, x, verbose=False):
        ...
        loss = (u_x - self.w_list[0]*torch.cos(self.w_list[0] * x) - self.w_list[1]*torch.cos(self.w_list[1] * x)).square().mean()
